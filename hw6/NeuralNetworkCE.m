...
stepSize=0.005;
nAvg=1000;
[W1,W2,accuracyClassification,trainError]=trainNNCE(trainData,trainLabel,nHid,nOut,2,maxIter,stepSize,nAvg);
[errorRate,predictLabel,nnOutput]= predictNN(W1,W2,validationData,validationLabel);
test_label=zeros(nTest,1);
[errorRateK,predictLabelK,nnOutput]= predictNN(W1,W2,test_data,test_label);
% plot the results
F3=figure;
plot(1:maxIter, trainError);
title('Training Error(Cross Entropy)');
xlabel('Iteration');
...
title('Classification Accuracy(CrossEntropy)');
xlabel('Iteration');
ylabel('Classification Accuracy Rate');
saveas(F4,'CE_accu.jpg');
